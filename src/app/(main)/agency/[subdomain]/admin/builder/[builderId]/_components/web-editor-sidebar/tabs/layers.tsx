    ...
    return (
      <ul className="w-full space-y-0.5">
        {elements.map((element) => {
          ...
          const selectedClass = isSelected ? 'bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground ' : ''
            ...
            return (
              <li key={element.id} className="w-full relative">
                <AccordionItem value={element.id} className="border-none">
                  <AccordionTrigger
                    className={`group text-sm hover:bg-sidebar-accent/50 hover:text-sidebar-accent-foreground py-0.5 rounded-md transition-all animate-fade-in hover:no-underline ${selectedClass}`}
